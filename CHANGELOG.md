...
## [v0.4.1] — 2026-03-14 · 收敛判断双重保障修复

### 修复
- **run.py 收敛判断**：原来只检查 `verdict=="passed"`，不检查分数，导致歌者说通过但分数79<85时错误收敛（🔴 严重）
- **finalize_epoch 跨纪元污染**：`best` 分数取了所有纪元最高分而非当前纪元，导致终章显示错误分数
- **fix**：收敛条件改为 `verdict=="passed" AND total >= CONVERGENCE_SCORE`，finalize 只取当前纪元分数
